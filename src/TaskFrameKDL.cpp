...
TaskFrameKDL::TaskFrameKDL(const KDL::Chain& chain, const JointIndexMap& joint_index_map)
{
    chain_ = chain;
    ...
    tf_name_ = chain.segments[chain_.getNrOfSegments()-1].getName();
}

TaskFrameKDL::~TaskFrameKDL(){
    delete pos_fk_solver_;
    delete jac_solver_;
}

void TaskFrameKDL::update(const base::samples::Joints &status){
